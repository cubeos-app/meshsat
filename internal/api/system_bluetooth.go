...
//
// This file shells out to host-side tooling (bluetoothctl, rfkill) and
// uses nsenter to bootstrap bluetoothd if it's not running. The CubeOS
// HAL-boundary rule normally forbids that in the API container, but
// field kits run meshsat as the sole trusted container on a dedicated
// Pi — there is no HAL container to go through and the least-privilege
// argument doesn't apply. The pre-commit / consistency-check treats the
// pragma above as the explicit opt-in for this file.
package api
