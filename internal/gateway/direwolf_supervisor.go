...
// MeshSat APRS integration: one channel, AFSK 1200, KISS on the given
// port, no AGW, no IGate (MeshSat handles APRS-IS directly via the
// gateway).
//
// Direwolf's KISSPORT directive only accepts a port number — not an
// IP:PORT — and binds INADDR_ANY unconditionally. We therefore enforce
// loopback-only binding at the C source level via a Dockerfile sed patch
// (see `direwolf-builder` stage in Dockerfile). With that patch the
// `KISSPORT 8001` line here results in a listener on 127.0.0.1:8001 only.
// [MESHSAT-517]
